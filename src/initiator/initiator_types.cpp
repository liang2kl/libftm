...
struct ftm_config *alloc_ftm_config(const char *interface_name,
                                    ...
                                    int peer_count) {
    struct ftm_config *config = (ftm_config *) malloc(sizeof(struct ftm_config));
...
struct ftm_peer_attr *alloc_ftm_peer() {
    struct ftm_peer_attr *peer = (ftm_peer_attr *) malloc(sizeof(struct ftm_peer_attr));
    memset(peer->flags, 0, sizeof(peer->flags));
    return peer;
}

struct ftm_results_wrap *alloc_ftm_results_wrap(struct ftm_config *config) {
    struct ftm_results_wrap *results_wrap = (ftm_results_wrap *)
            malloc(sizeof(struct ftm_results_wrap));
    results_wrap->results = (ftm_resp_attr **)
...
struct ftm_resp_attr *alloc_ftm_resp_attr() {
    struct ftm_resp_attr *resp_attr = (ftm_resp_attr *) malloc(sizeof(struct ftm_resp_attr));
